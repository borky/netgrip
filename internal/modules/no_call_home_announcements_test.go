...
//
// The announcements feed is the one request this panel makes to the
// project's own server, and the only one cmd/netgrip/no_call_home_test.go
...
var allowedAnnouncementHeaders = map[string]string{
	"User-Agent":      "netgrip-announcements",
...
func TestTheAnnouncementsFetchCarriesNothingIdentifying(t *testing.T) {
	...
	t.Setenv("NETGRIP_ANNOUNCEMENTS_URL", srv.URL+"/announcements.json")
	StartAnnouncements()
